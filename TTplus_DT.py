...
Version 5.0 - 2026-03-27
-updated to handle 4D and 45 correctly.
DTmax calculation only during daily hours
Corrected the calibration functions both for SFD1 and SFD2 
...
TreeType = ["diffuse-porous", "dluhosch_10-50"]
...
# analyzing cloud data string type 4B
                ...
                # Perform the calculation or any desired action here
                    ...
                    s = pd.to_datetime(df4B['timestamp'], unit='s', utc=True).dt.tz_convert(timezone)
                    s.dt.strftime('%Y-%m-%d %H:%M:%S %Z%z')  # shows CET vs CEST and offset
...
# _________________CALLING DIFF DVICE TYPE______________________________________________________________________________________________
                ...
                if '45' in dfall[3].values:
                    ...
                    #Rimuove airT dalla analisi perchè non subisce conversione ADC
                    ...
                    for i in range(len(df45)):
                        ...
                        df45[i] = df45[i].sort_values('date').reset_index(drop=True)
                        ...
                        df45[i].loc[df45[i]['dT1'] < 0, 'dT1'] = np.nan
                        df45[i].loc[df45[i]['dT1'] > 10, 'dT1'] = np.nan
                        

                        # --- Set datetime index for rolling operations ---
                        df45[i] = df45[i].sort_values('date').set_index('date')     

                        # Calculate dTmax using a rolling window of 12 hours before and after
                        df45[i]['dTmax1'] = df45[i]['dT1'].rolling('24H', center=True).max()
                        #'''

                        # --- dTmax1_filtered: rolling max using NIGHTTIME data only (8pm–8am) ---
                        # Nighttime = hours outside [8, 20), i.e. hour < 8 OR hour >= 20
                        night_mask = (df45[i].index.hour < 8) | (df45[i].index.hour >= 20)
                        dT1_night = df45[i]['dT1'].where(night_mask, other=np.nan)
                        df45[i]['dTmax1_filtered'] = dT1_night.rolling('24H', center=True).max()

                        df45[i] = df45[i].reset_index()
                        
                        
                        df45[i]['K1_hr'] = (( df45[i]['dTmax1_filtered']- df45[i]['dT1'])/ df45[i]['dT1']). round(2)
                        ...
                        dT2_night = df45[i]['dT2'].where(night_mask, other=np.nan)
                        df45[i]['dTmax2_filtered'] = dT2_night.rolling('24H', center=True).max()
                        df45[i] = df45[i].reset_index()
                        
                        
                        df45[i]['K1_h'] = ((df45[i]['dTmax2_filtered']- df45[i]['dT2'])/ df45[i]['dT2']). round(2)
                        ...
                        if data_freq == 'tree':
                            ...
                            '''
                            2. AGGIUNGERE LA SPECIE NUOVA. 
                            Dalla table2 del paper https://academic.oup.com/treephys/article/45/6/tpaf047/8114617?login=false#523285988

                            includere i coefficienti K1, 10/50

                            K1_hr = (dTmax1 - dT1) / dT1 both heater and ref probe
                            K1_h = (dTmax2 - dT2) / dT2 only heater probe

                            apply K1 coefficients to both SFD1 and SFD2


                            '''

                            if species == 'diffuse-porous':
                                  df45[i]['SFD1'] = 5.5750 * df45[i]['K1_hr']                               
                                  df45[i]['SFD2'] = 5.5750 * df45[i]['K1_h']

                            elif species == 'dluhosch_10-50':
                                  
                                  df45[i]['SFD1'] = 4.65 * df45[i]['K1_hr']      #a = 4.65+-0.26              
                                  df45[i]['SFD2'] = 4.65 * df45[i]['K1_h']

                            else:
                                  # Granier calibration coefficients
                                  #alpha = 67.99735
                                  alpha = 119                              
                                  #beta = 1.18245
                                  beta = 1.231
                                  df45[i]['SFD1'] = alpha * (df45[i]['K1_hr']**beta)  # g/m^2/s
                                  df45[i]['SFD2'] = alpha * (df45[i]['K1_h']**beta) # g/m^2/s
                ...
                try:
                    if 'df45' in locals() and df45:  # Check if `df45` is defined and not empty
                        ...
                        col= ['Vbat', 'airT', 'RH', 'dT1','dTmax1','dTmax1_filtered','dT2','dTmax2','dTmax2_filtered','SFD1','SFD2','dis','yaw','pitch','roll']

                        # Define a dictionary for units if available
                        units= {'Vbat': 'Battery voltage (mV)', 'airT': 'Air Temperature (°C)', 'RH': 'relative humidity (%)',
                                'dT1': 'Temp difference using both reference and heater (°C)','dTmax1': 'max value of DT1 over a 24 hour window (°C)', 'dTmax1_filtered': 'max value of DT1 over a 24 hour window (°C). values between 8pm and 8am only',
                                'dT2': 'Temp difference using heater only (°C)', 'dTmax2': 'max value of DT2 over a 24 hour window (°C)', 'dTmax2_filtered': 'max value of DT2 over a 24 hour window (°C). values between 8pm and 8am only',
                                'SFD1': 'Sap flux density_TDM (g*m-2*s-1)', 'SFD2': 'Sap flux density_TDM using only heater (g*m-2*s-1)', 'dis': 'growth sensor distance from bark (mm)', 
                                'yaw': 'stability-Z (°)', 'pitch': 'stability-Y (°)', 'roll': 'stability-X (°)'}
